...
public class JwtService {
  ...
  public String getToken(String memberId, String role, Long member_no) {
    String token = Jwts.builder().setSubject(memberId).claim("role", role).claim("member_no", member_no)
  ...
  public String getAuthUser(HttpServletRequest request) {
    ...
    if (token != null) {
      String user = Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(token.replace(PREFIX, "")).getBody()
          .getSubject();
      if (user != null)
        return user;
    }
  ...
  /**
   * 요청의 Authorization 헤더에서 토큰을 파싱해 role 클레임을 꺼내 권한 반환
   */
