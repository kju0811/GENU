            ...
            <div className="overflow-x-auto rounded-box border border-base-content/5 bg-base-100" style={{width:'60%'}}>
              <table className="table">
                ...
                {currentItems.map((item) => (
                  <tr 
                  key={item.announce_no} onClick={() => navigate(`/announce_read/${item.announce_no}`)} 
                  style={{cursor:'pointer'}} className="transition-transform hover:scale-[1.02]"
                  > 
                    <th style={{width:'20%',paddingLeft: '3%'}}>{item.announcetitle}</th>
                    <td style={{width:'50%'}}> 
